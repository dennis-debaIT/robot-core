...
- **Liga Event-Ticker Nachspielzeit**: Toreinträge im Live-Ticker zeigen jetzt `45+2'` statt nur `45'` wenn ein `injuryTime`-Feld vorhanden ist. Beide Datenquellen (football-data.org und OpenLigaDB) liefern das Feld. Minute-0-Bug (falsy-Check) behoben.
- **News: Artikel-Reader-Modus**: Klick auf einen News-Eintrag öffnet ein Vollbild-Overlay im aktuellen Display-Theme. Der Backend-Endpoint `GET /news/article?url=…` lädt die Seite mit Browser-User-Agent und extrahiert per `trafilatura` den bereinigten Artikeltext (kein Iframe, keine Werbung, kein Cookie-Banner). RSS-Teaser (`description` / `content:encoded`) wird sofort angezeigt während der Volltext lädt. Schlägt trafilatura fehl (DSGVO-Consent-Wall), erscheint ein Button „🌐 Im Browser öffnen" der die Originalseite als Iframe lädt. Schließen-Button immer im Overlay sichtbar (kein Browser-Zurück nötig im Vollbild-Modus).
...
- **Einkaufsliste**: Neues Modul — Einkaufsliste im Display (Nav-Button 🛒 „Einkauf"), Artikel hinzufügen/abhaken/löschen, automatischer Hintergrund-Sync (alle 60 s) mit dem neuen `erika-sync-server` auf Port 9000. Robot-Core pusht lokale Änderungen und pulled neue Einträge der Android-App via Delta-Sync (`?since=<ISO>`). Offline-fähig: lokale SQLite-Tabelle bleibt Quelle, Sync läuft still im Hintergrund. Admin: Modul ein-/ausschalten + manuellen Sync-Trigger. Konfiguration via `.env`: `SHOPPING_SYNC_URL` + `SHOPPING_SYNC_TOKEN`.
...
### Neu
- **WM/EM Turnier-Modus (Community)**: Neuer optionaler Display-Modus, der das Gesichts-Center-Panel mit dem aktuellen Spieltag des laufenden Turniers überlagert. Konfigurierbar im Admin unter „Turnier" (API-Key von football-data.org, Wettbewerbs-Code wie „WC"/„EC", Ein-/Abschalten). Solange kein Spiel läuft, wird der aktuelle Spieltag mit Ergebnissen und Anstoßzeiten angezeigt; wenn ein Spiel live ist, wechselt die Anzeige automatisch zur Live-Ansicht. Bei jedem Tor erscheint 5 Sekunden lang eine Torjubel-Animation (⚽ + „TOOOOR!") im Vollbild. Das Feature steht in der Community-Edition kostenlos zur Verfügung.
- **Geschlecht pro Person (Admin)**: Im Personen-Tab des Admins lässt sich pro Person „Mann"/„Frau"/„–" wählen. Je nach Auswahl zeigt die Personenliste im Hausaufgaben-Modul (und der Admin-Avatar) das passende Symbol (🙋‍♂️/🙋‍♀️ statt 🙋). Außerdem fließt die Information in den LLM-Kontext ein: das System-Prompt nennt explizit, dass die aktuell erkannte/adressierte Person männlich bzw. weiblich ist, inkl. Hinweis auf passende Anredeformen.
- **Hausaufgaben-Modul (Plus/Family)**: Neuer Navigationspunkt „🧹 Hausaufgaben" (zwischen „Start" und „Fahrzeuge", über die Modul-Liste ein-/ausblendbar) für die Verwaltung von Haushaltsaufgaben. Im Admin lassen sich beliebig viele Aufgaben (Name + Icon) anlegen, sortieren und löschen (Soft-Delete, Historie bleibt erhalten). Im Display zeigt die linke Liste alle Aufgaben; ein Klick öffnet im Center eine Statistik mit Tabs „Woche"/„Monat"/„Jahr" (analog zum Ladeverlauf des Fahrzeugs) — Erledigungen pro Person als Balkendiagramm (eigene Farbe je Person, Achse nur in ganzen Zahlen, da Erledigungen nicht gebrochen sein können) plus „Wochensieger dieser Aufgabe". Ein Klick auf eine Person (in den Totals oder im Diagramm) zeigt deren einzelne Erledigungen mit Zeitstempel; jeder Eintrag (sowie die allgemeine „Letzte Erledigungen"-Liste) lässt sich einzeln löschen, falls sich jemand verklickt hat. Die rechte Liste zeigt alle Personen aus den bestehenden Personenprofilen; ein Klick darauf loggt sofort eine Erledigung für die gerade ausgewählte Aufgabe. Zusätzlich gibt es einen separat schaltbaren Gesamt-Wochensieger über alle Aufgaben hinweg. Sprachsteuerung ist bewusst noch nicht enthalten (siehe Roadmap).
...
### Tests
- Neue Tests für das Geschlecht-Feld (`test_person_gender_api.py`): `set_gender`/`get_person`/`list_people`, Ablehnung ungültiger Werte, sowie das System-Prompt enthält den Geschlechtshinweis für die aktuell adressierte Person.
- Neue Tests für das Hausaufgaben-Modul (`test_chore_service.py`): Aufgaben-CRUD inkl. Soft-Delete (inaktive Aufgaben verschwinden aus `list_tasks()`, Completions bleiben zuordenbar), `log_completion` + `task_stats` (Counts pro Person, Ermittlung des „Wochensiegers") für Woche/Monat/Jahr inkl. Kalendergrenzen, `overall_weekly_stats()` über mehrere Aufgaben hinweg, sowie `list_completions()` (Filter nach Periode/Person, Sortierung) und `delete_completion()`.
