...
### Neu
- **Hausaufgaben-Modul (Plus/Family)**: Neuer Navigationspunkt „🧹 Hausaufgaben" (zwischen „Start" und „Fahrzeuge", über die Modul-Liste ein-/ausblendbar) für die Verwaltung von Haushaltsaufgaben. Im Admin lassen sich beliebig viele Aufgaben (Name + Icon) anlegen, sortieren und löschen (Soft-Delete, Historie bleibt erhalten). Im Display zeigt die linke Liste alle Aufgaben; ein Klick öffnet im Center eine Statistik mit Tabs „Woche"/„Monat"/„Jahr" (analog zum Ladeverlauf des Fahrzeugs) — Erledigungen pro Person als Balkendiagramm plus „Wochensieger dieser Aufgabe". Die rechte Liste zeigt alle Personen aus den bestehenden Personenprofilen; ein Klick darauf loggt sofort eine Erledigung für die gerade ausgewählte Aufgabe. Zusätzlich gibt es einen separat schaltbaren Gesamt-Wochensieger über alle Aufgaben hinweg. Sprachsteuerung ist bewusst noch nicht enthalten (siehe Roadmap).
